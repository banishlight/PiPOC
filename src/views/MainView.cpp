...
static constexpr int   GEAR_X          = W - 40;
static constexpr int   GEAR_Y          = H - BOTBAR_H + 8;
...
void MainView::draw() {
    ...
    DrawTextEx(Assets::catFont16, clockBuf, {(float)(W - cw - 16), 10}, 16, 1, TEXT_DIM);
    DrawRectangle(W - cw - 32, 8, 1, 20, BORDER);
    DrawTextEx(Assets::catFont16, "BT  CONNECTED", {(float)(W - cw - 148), 10}, 16, 2, TEXT_DIM);
    DrawRectangle(W - cw - 164, 8, 1, 20, BORDER);
    DrawTextEx(Assets::catFont16, "ECU  ONLINE", {(float)(W - cw - 268), 10}, 16, 2, TEXT_DIM);

    // Title block
    int titleY = TOPBAR_H + 60;
    const char* title = "RX-8";
    int tw = (int)MeasureTextEx(Assets::catFont52, title, 52, 4).x;
    DrawTextEx(Assets::catFont52, title, {(float)((W - tw) / 2), (float)titleY}, 52, 4, TEXT_BRIGHT);
    ...
    DrawTextEx(Assets::catFont16, sub, {(float)((W - sw) / 2), (float)(titleY + 72)}, 16, 8, TEXT_DIM);
    ...
    DrawTextEx(Assets::catFont16, "CUBA'S IPHONE", {34, (float)(H - BOTBAR_H + 11)}, 16, 2, TEXT_DIM);

    // Version
    const char* ver = "v0.1.0  PIPOC";
    int vw = (int)MeasureTextEx(Assets::catFont16, ver, 16, 2).x;
    DrawTextEx(Assets::catFont16, ver, {(float)(W - vw - GEAR_SIZE - 24), (float)(H - BOTBAR_H + 11)}, 16, 2, {34, 34, 34, 255});
